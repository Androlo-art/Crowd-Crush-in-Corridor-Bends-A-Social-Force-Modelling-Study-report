...
FIELD_ROOT = os.path.join(ROOT, '..', 'results', 'bend_obstacle_navfields')        # PILLAR-AWARE per-config fields (make_field_obstacle.py)
...
def _check_field(angle, label):
    """PILLAR-AWARE field for THIS config: navfields/<label>/ff_field_a<ang>_r<RTAG>.csv."""
    fdir = os.path.join(FIELD_ROOT, label)
    gj = os.path.join(fdir, 'geometry.json')
    if not os.path.isdir(fdir) or not os.path.exists(gj):
        raise ValueError(f'missing pillar-aware field folder for "{label}": {fdir} '
                         f'(run make_field_obstacle.py first)')
    meta = json.load(open(gj))
    egress = meta.get('egress', None)
    if egress is None or abs(float(egress) - float(DOOR)) > 0.05:
        raise ValueError(f'egress mismatch in {gj}: {egress} != {DOOR}')
    if not meta.get('pillar_aware', False):
        raise ValueError(f'{gj} is not a PILLAR-AWARE field (run make_field_obstacle.py)')
    ff = os.path.join(fdir, f'ff_field_a{int(round(angle))}_r{RTAG}.csv')
    if not os.path.exists(ff):
        raise ValueError(f'missing field {ff} for angle {angle}, config "{label}"')
    return fdir, ff
...
def run_one(task, snapdir=None):
    ...
    try:
        field_dir, ff = _check_field(angle, label)
...
def main():
    ...
    ap.add_argument('--tag', default='obsnav')
    args = ap.parse_args()
    ...
    for a in angles:
        for ci in cfg_idx:
            try: _check_field(a, COLCFGS[ci]['label'])
            except ValueError as ex: problems.append(str(ex))
    if problems:
        raise SystemExit('FIELD PRE-FLIGHT FAILED ({} issue(s)):\n  '.format(len(problems))
                         + '\n  '.join(problems[:12]))
